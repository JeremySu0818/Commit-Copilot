...
export const arPrompt: LocalePromptBundle = {
  ...
  diffCoverageRequirement:
    'تم تفعيل التغطية الكاملة للفروقات. قبل استدعاء `write_commit_message`، يجب استدعاء `get_diff` بنجاح لكل ملف تم تغييره. يمكنك تضمين عدة ملفات في مصفوفة `paths` واحدة.',
  ...
  toolDescGetDiffBatch:
    '- صيغة الدفعة: يقبل `get_diff` مصفوفة `paths` غير فارغة بدلاً من `path` ويعيد الفرق الكامل والدقيق لكل ملف مطلوب.',
  toolDescReadFile:
