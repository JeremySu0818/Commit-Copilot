...
export const ruPrompt: LocalePromptBundle = {
  ...
  diffCoverageRequirement:
    'Полное покрытие различий включено. Перед вызовом `write_commit_message` необходимо успешно вызвать `get_diff` для каждого изменённого файла. В одном массиве `paths` можно указать несколько файлов.',
  ...
  toolDescGetDiffBatch:
    '- Пакетная форма: `get_diff` принимает непустой массив `paths` вместо `path` и возвращает полный точный diff каждого запрошенного файла.',
  toolDescReadFile:
