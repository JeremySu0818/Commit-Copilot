...
export const ruPrompt: LocalePromptBundle = {
  ...
  toolDescGetDiffBatch:
    '- Пакетная форма: `get_diff` принимает непустой массив `paths` вместо `path`; если это компактнее, структурно согласованные патчи возвращаются как проверенная harness сводка с одним репрезентативным exact diff.',
  diffInvestigationPlanTitle: '## План исследования Harness Diff',
  diffInvestigationPlanBody:
    'Harness проверяет каждый exact patch и сравнивает только общую структуру изменённых строк; назначение и семантика файлов не выводятся. Используйте `get_diff` с `paths` группы, чтобы получить проверенную сводку и один репрезентативный exact diff. Дополнительные, отсутствующие, переставленные или иначе структурированные правки остаются отдельными. Переменное содержимое других участников опускается для сокращения input token; группа не доказывает одинаковый смысл или равенство опущенных значений.',
  diffInvestigationPlanGroup:
    '- G{0} ({1} структурно согласованных файлов; представитель: {2}): {3}',
  diffInvestigationPlanStandalone:
    'Структурно отличающиеся файлы (проверяйте отдельно или явным пакетом):',
  toolDescReadFile:
