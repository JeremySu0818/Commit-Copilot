...
export const esLocale: LocaleTextBundle = {
  commitCopilotErrorMessages: {
    ...
    'rewrite.upstreamVerifyFailed': (args) =>
      'No se puede verificar el upstream "{upstreamRef}" después del fetch.'.replace(
        '{upstreamRef}',
        args.upstreamRef ?? '',
      ),
    'rewrite.remoteNotIntegrated': (args) =>
      'No se puede reescribir de forma segura porque el HEAD local no incluye el último {upstreamRef} ({remoteHash}). Ejecute primero git pull --rebase (o merge).'
        .replace('{upstreamRef}', args.upstreamRef ?? '')
        .replace('{remoteHash}', args.remoteHash ?? ''),
    'rewrite.autoSyncMissingUpstream': () =>
      'No se puede sincronizar automáticamente sin una rama upstream. Configure el upstream primero.',
    'rewrite.autoSyncUpstreamUnavailable': (args) =>
      'No se puede sincronizar automáticamente porque el upstream "{upstreamRef}" no está disponible después del fetch.'.replace(
        '{upstreamRef}',
        args.upstreamRef ?? '',
      ),
    'rewrite.autoSyncUnsafeRemoteRewrite': (args) =>
      'No se puede sincronizar automáticamente de forma segura porque el upstream "{upstreamRef}" ya no contiene {previousHash}.'
        .replace('{upstreamRef}', args.upstreamRef ?? '')
        .replace('{previousHash}', args.previousHash ?? ''),
    'rewrite.forcePushStaleInfo': () =>
      'Información obsoleta de force-with-lease: la referencia de seguimiento remoto cambió antes del fallback de VS Code.',
  },
  errorMessages: {
  ...
  extensionText: {
    output: {
      ...
      rewriteStart: (timestamp) =>
        `[${timestamp}] Iniciando generación de reescritura de commit-copilot...`,
      rewriteCancelRequestedFromProgress:
        'Cancelación solicitada desde la interfaz de progreso.',
      rewriteCommitRewritten: (originalHash, replacementHash) =>
        `Commit reescrito: ${originalHash} -> ${replacementHash}`,
      rewriteReplacementCommitFallback: 'actualizado',
      ...
      rewriteAutoSyncPreviewSummary: (upstreamRef) =>
        `Vista previa de sincronización automática para ${upstreamRef}:`,
      rewriteAutoSyncRemoteTracking: (beforeHash, afterHash) =>
        `Seguimiento remoto: ${beforeHash} -> ${afterHash}`,
      rewriteAutoSyncLocalHead: (headHash) =>
        `HEAD local reescrito: ${headHash}`,
      rewriteAutoSyncCommitsToPush: 'Commits que serán empujados:',
      rewriteAutoSyncNoCommitsToPush: '(ninguno)',
      rewriteAutoSyncDiffStat: 'Diff que será empujado:',
      rewriteAutoSyncNoDiffStat: '(sin diferencias)',
      rewriteAutoSyncRetryUsesCurrentLease:
        'El reintento de empuje utilizará el lease actualizado del upstream, no el hash previo a la reescritura.',
      rewriteCliAuthFailedUsingVscodeFallback:
        'El empuje CLI falló porque Git necesita credenciales; reintentando a través de VS Code Git.',
      rewriteVscodeFallbackSkippedLeaseChanged:
        'Se omitió el fallback de VS Code Git porque la referencia de seguimiento remoto cambió.',
    },
    notification: {
      gitExtensionMissing:
      ...
      rewriteCommitNoSubject: '(sin asunto)',
      rewriteCommitRootDescription: 'commit raíz',
      rewriteCommitMergeDescription: 'commit de merge',
      rewriteCommitParentDescription: (parentHash) => `padre ${parentHash}`,
      rewriteCommitSelectTitle: 'Seleccionar commit para reescribir',
      rewriteCommitSelectPlaceholder:
        'Elige un commit del historial de la rama actual',
      rewriteWorkspaceDirtyBoth:
        'No se puede reescribir el historial de commits mientras haya cambios staged (sin commit) y modified (unstaged). Haz commit o stash primero.',
      rewriteWorkspaceDirtyStaged:
        'No se puede reescribir el historial de commits mientras haya cambios staged (sin commit). Haz commit o stash primero.',
      rewriteWorkspaceDirtyUnstaged:
        'No se puede reescribir el historial de commits mientras haya cambios modified (unstaged). Haz commit o stash primero.',
      rewriteProgressTitle: (providerName) => `Reescritura (${providerName})`,
      rewriteAnalyzingCommit: (shortHash) =>
        `Analizando commit ${shortHash}...`,
      commitMessageCannotBeEmpty: 'El mensaje de commit no puede estar vacío.',
      rewriteApplyingTitle: (shortHash) => `Reescribiendo ${shortHash}`,
      rewriteApplyingProgress: 'Reescribiendo historial de commits...',
      rewriteFailedHistory: 'No se pudo reescribir el historial de commits.',
      rewriteCommitMessageRewritten: (shortHash) =>
        `Mensaje del commit ${shortHash} reescrito.`,
      rewriteDetachedHeadPushUnavailable:
        'El historial de commits se reescribió, pero force push with lease no está disponible en estado detached HEAD.',
      rewriteForcePushPrompt: (target) =>
        `Historial reescrito. ¿Hacer force push with lease a ${target}?`,
      pushWithLeaseConfirmAction: 'Push with Lease',
      rewriteForcePushCompleted: (target) =>
        `Force push with lease completado: ${target}.`,
  ...
  webviewLanguagePack: {
    ...
    labels: {
      ...
      selectedCommitMessage: 'Mensaje de commit seleccionado',
    },
    placeholders: {
    ...
    statuses: {
      ...
      commitMessageCannotBeEmpty: 'El mensaje de commit no puede estar vacío.',
      pushingWithLease: 'Haciendo push with lease...',
      forcePushWithLeaseCompleted: 'Force push with lease completado.',
      forcePushWithLeaseFailed: 'Force push with lease falló.',
    },
    descriptions: {
