...
export const DISPLAY_LANGUAGE_OPTIONS: LanguageOption[] = [
  {
    value: 'auto',
    labels: { en: 'Auto (Follow VS Code)', 'zh-TW': '自動（跟隨 VS Code）', 'zh-CN': '自动（跟随 VS Code）', ja: '自動（VS Code に従う）', ko: '자동 (VS Code 설정 따름)', es: 'Automático (según VS Code)', ar: 'تلقائي (حسب VS Code)', cs: 'Automaticky (podle VS Code)', nl: 'Automatisch (volgens VS Code)' },
  },
...
export function normalizeDisplayLanguage(value: unknown): DisplayLanguage {
  if (value === 'en' || value === 'zh-TW' || value === 'zh-CN' || value === 'ja' || value === 'ko' || value === 'es' || value === 'ar' || value === 'cs' || value === 'nl' || value === 'auto') {
    return value;
  }
...
export function resolveEffectiveDisplayLanguage(
  ...
  if (displayLanguage === 'en' || displayLanguage === 'zh-TW' || displayLanguage === 'zh-CN' || displayLanguage === 'ja' || displayLanguage === 'ko' || displayLanguage === 'es' || displayLanguage === 'ar' || displayLanguage === 'cs' || displayLanguage === 'nl') {
