...
test('fillTemplate replaces unknown placeholders with empty text', () => {
  const output = fillTemplate('Hello {name}, {missing}!', { name: 'Commit' });
  assert.equal(output, 'Hello Commit, !');
});

test('renderStatusHtml escapes status text', () => {
...
test('normalizeGenerateMode defaults to agentic', () => {
...
test('normalizeCommitOutputOptions keeps booleans and falls back to defaults', () => {
  const defaults = {
    includeScope: true,
    includeBody: false,
    includeFooter: true,
...
test('normalizeMaxAgentStepsValue returns positive integer or zero', () => {
...
test('normalizeOllamaHostValue trims custom host and falls back to default', () => {
  assert.equal(
