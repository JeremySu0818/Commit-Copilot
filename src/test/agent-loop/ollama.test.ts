...
void test(
  'runOllamaAgentLoop pulls model, reports progress, and generates from inline diff',
  async () => {
    ...
    const chatMessages = isRecord(chatRequests[0]) ? chatRequests[0].messages : null;
...
void test(
  'runOllamaAgentLoop maps connection failures to friendly host error',
  async () => {
